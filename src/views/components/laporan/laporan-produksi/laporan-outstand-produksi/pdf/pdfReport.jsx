...
const pdfReport = (data = "") => {
  ...
  let finalY = 30;
  doc.text(`Laporan Outstand Produksi (${datahead.divisi})`, 14, 15);
  doc.setFontSize(20);
  doc.text("PRODUKSI", 200, 15);

  doc.setFontSize(10);
  doc.setProperties({
    title: `Outstand Produksi (${datahead.divisi})`,
  });
  doc.text(`Kode Staff : ${datahead.kode_staff}`, 14, 25);
  ...
  data.forEach((element) => {
    const row = [
      ...
      {
        content: parseFloat(element.stock_akhir) ? element.stock_akhir : 0,
        styles: {
          halign: "right",
        },
      },
      {
        content: parseFloat(element.berat_akhir) ? element.berat_akhir : 0,
        styles: {
          halign: "right",
        },
      },
      {
        content: parseFloat(element.kadar) ? element.kadar : 0,
        styles: {
          halign: "right",
        },
      },
  ...
  const footer = [
    ...
    {
      content: data.reduce((a, b) => a + (parseFloat(b.stock_akhir) || 0), 0),
      styles: {
        halign: "right",
      },
    },
  ...
  x.document.write(
    `<html>
    <head>
    <title>Outstand Produksi(${datahead.divisi})</title>
    </head>
