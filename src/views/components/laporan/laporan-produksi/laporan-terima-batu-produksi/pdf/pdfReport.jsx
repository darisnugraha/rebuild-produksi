...
const pdfReport = (data = "") => {
  ...
  const datahead = JSON.parse(
    localStorage.getItem("laporan_terima_batu_produksi")
  );
  let tableRows = [];
  let tableColumn = [];

  let finalY = 30;
  doc.text(`Laporan Terima Batu Produksi (${datahead.nama_divisi})`, 14, 15);
  doc.setFontSize(20);
  doc.text("PRODUKSI", 200, 15);

  doc.setFontSize(10);
  doc.setProperties({
    title: `Terima Batu Produksi (${datahead.nama_divisi})`,
  });
  ...
  dataGroupArr.forEach((element) => {
    ...
    const rowfooter = [
      ...
      {
        content: element.reduce((a, b) => a + parseFloat(b.stock_in), 0),
        styles: {
          halign: "right",
        },
      },
  ...
  x.document.write(
    `<html>
    <head>
    <title>Terima Batu Produksi (${datahead.nama_divisi})</title>
    </head>
