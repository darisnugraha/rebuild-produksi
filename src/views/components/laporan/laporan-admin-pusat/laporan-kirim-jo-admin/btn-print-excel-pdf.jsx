...
const BtnPrint = () => {
  ...
  const dataKirimJoAdmin = useSelector(KirimJoAdmin.getAllLaporanKirimJoAdmin);
  const datahead = JSON.parse(localStorage.getItem("laporan_kirim_jo_admin"));

  const pdfExportHandle = () => {
    pdfReport(dataKirimJoAdmin);
  };

  return (
    <Row style={{ marginTop: 15 }}>
      <Col span={10} style={{ marginTop: 10 }}>
        <ExcelReport dataExel={dataKirimJoAdmin} dataHead={datahead} />
      </Col>
