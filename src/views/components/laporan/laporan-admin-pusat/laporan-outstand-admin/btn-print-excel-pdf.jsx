...
const BtnPrint = () => {
  ...
  const dataOutstandAdmin = useSelector(
    OutstandAdmin.getAllLaporanOutstandAdmin
  );
  const datahead = JSON.parse(localStorage.getItem("laporan_outstand_admin"));

  const pdfExportHandle = () => {
    pdfReport(dataOutstandAdmin);
  };

  return (
    <Row style={{ marginTop: 15 }}>
      <Col span={10} style={{ marginTop: 10 }}>
        <ExcelReport dataExel={dataOutstandAdmin} dataHead={datahead} />
      </Col>
